...
namespace Pipeline
{
    public partial class MainWindow : Form
    {
        ...
        private static ISolrOperations<SolrData> solr = null;
        ...
        public static void insertintodb(string title, string _abstract, string link, string date, string tag, int answer)
        {
            ...
            _postdate = date;
            if (tag != null)
                tag = tag.Substring(0, (int)Math.Min(60, tag.Length)).Replace("\"", "").Replace("'", ""); string selectSQL = string.Format("select tid from XueBa.dbo.Tag where (name = '{0}')", tag);
        ...
        private void Process_Sig_Click(object sender, EventArgs e)
        {
            ...
            else if (temp.Extension.Equals(".doc"))
            {
                try
                {
                    ...
                    string temp2 = doc.Content.Text;
                    //Console.WriteLine(temp);
                    app.Quit(ref unknow, ref unknow, ref unknow);
                    ProcessProcedure.Processwrd(temp1,temp2, false);
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.Message);
           ...
           if (web.Filepath != null && web.IsDeal == 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else if (ProcessProcedure.msdn_channel9(curUrl))
                        {
                            ProcessProcedure.msdn_channel9process(curPath, curUrl);
                        }
                        else
                        {
